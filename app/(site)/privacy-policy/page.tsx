...
export async function generateMetadata() {
  ...
  return createPageMetadata({
    title: `${config.legal.privacyPolicy.title} | ${config.name}`,
    description: formatPageTitleText(config.pageTitles.privacyDescription, config.name),
    path: "/privacy-policy",
    config,
  });
}
...
export default async function PrivacyPolicyPage() {
  const config = await getSiteConfig();
  const page = config.legal.privacyPolicy;
    ...
    <>
      <JsonLd
        data={createBreadcrumbSchema([
          ...
          { name: crumbs.privacy, url: "/privacy-policy" },
        ], config)}
      />
